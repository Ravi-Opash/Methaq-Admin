...
const HealthQuoteSource = ({ dateStatus, healthQuoteSourceData, loading }) => {
  const [quotesSourceInfo, setQuotesSourceInfo] = useState([]);
  useEffect(() => {
    if (healthQuoteSourceData) {
      const arr = [...healthQuoteSourceData].find((i) => i.source == null);
      const isIncludeWeb = [...healthQuoteSourceData].find((i) => i.source == "Web");

      const array = [];
      healthQuoteSourceData?.map((item) => {
        let obj = item;
...
export default HealthQuoteSource;
