...
function AgentPerfomance({ handleClose, agentWiseList, agentData }) {
          ...
          <Box sx={{ minWidth: 700, mt: 3 }}>
            {agentWiseList?.proposalsPolicy ? (
              <Table>
                ...
                {agentWiseList?.proposalsPolicy?.length > 0 ? (
                  <TableBody>
                    <>
                      {agentWiseList?.proposalsPolicy?.map((ele, index) => {
                        return (
                          <TableRow key={index} sx={{ cursor: "default" }}>
                            <TableCell>
                              <Tooltip title="Visit Proposal">
                                <Typography
                                  component={Link}
                                  href={`/proposals/${ele?.policy?.quote?.proposalId}`}
                                  sx={{
                                    fontSize: 14,
                                    cursor: "pointer",
                                    width: "max-content",
                            ...
                            <TableCell>
                              <Tooltip title="Visit Policy">
                                ...
                                >
                                  {ele?.policy?.companyPolicyNumber || ele?.policy?.policyNumber || "0"}
                                </Typography>
                              </Tooltip>
                            </TableCell>
                            <TableCell>
                  ...
                  <Box sx={{ display: "flex", justifyContent: "center", my: 2, width: "100%" }}>No data found!</Box>
                )}
              </Table>
            ) : (
              <Box
